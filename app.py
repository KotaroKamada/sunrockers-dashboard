...
def safe_get_latest_and_previous_for_player(df, player_name, category, metric):
    """特定の選手の特定の項目の最新値と前回値を取得（修正版）"""
    try:
        # 該当選手の全データを取得
        player_all_data = df[(df['名前'] == player_name) & (df['カテゴリー'] == category)]
...
def calculate_z_score(player_val, category_data, column):
    """選手の値をカテゴリー平均を基準としたZスコアで評価（5段階評価）"""
    try:
        if player_val is None or category_data.empty or column not in category_data.columns:
            return 3  # デフォルト値
        ...
        category_mean = np.mean(valid_values)
        category_std = np.std(valid_values, ddof=1)
        
        if category_std == 0:
            return 3
        
        z_score = (player_val - category_mean) / category_std
...
def create_radar_chart(player_data, category_data, config):
    ...
    try:
        ...
        for metric in radar_metrics:
            ...
            if player_val is not None:
                z_score = calculate_z_score(player_val, category_data, metric)
...
def create_comprehensive_comparison_table(df, selected_players, category, available_metrics):
    """全項目の詳細比較テーブルを作成（修正版 - 項目ごとの最新・前回データを正確に取得）"""
    comparison_data = []
    ...
    for player_name in selected_players:
        ...
        if not player_all_data.empty:
            player_id = safe_get_value(player_all_data, 'ID', '')
            row_data['ID'] = player_id if player_id != '' else '-'
        else:
            row_data['ID'] = '-'
        
        # 各メトリクスについて最新値、前回値、変化、スコアを計算
        for metric in available_metrics:
            # 項目ごとの最新・前回データを取得
            latest_val, previous_val, latest_date, previous_date = safe_get_latest_and_previous_for_player(
                df, player_name, category, metric
            )
            
            # 最新値
            row_data[f'{metric}_最新値'] = format_value(latest_val, "", metric) if latest_val is not None else "-"
            
            # 前回値
            row_data[f'{metric}_前回値'] = format_value(previous_val, "", metric) if previous_val is not None else "-"
            ...
            if latest_val is not None and previous_val is not None:
                ...
                if metric == 'Fat%':
                    if diff > 0:
                        row_data[f'{metric}_変化'] = f"+{diff * 100:.2f}%"
                    elif diff < 0:
                        row_data[f'{metric}_変化'] = f"{diff * 100:.2f}%"
                    else:
                        row_data[f'{metric}_変化'] = "0.00%"
                else:
                    if diff > 0:
                        row_data[f'{metric}_変化'] = f"+{diff:.2f}"
                    elif diff < 0:
                        row_data[f'{metric}_変化'] = f"{diff:.2f}"
                    else:
                        row_data[f'{metric}_変化'] = "0.00"
            else:
                row_data[f'{metric}_変化'] = "-"
            
            # スコア
            if latest_val is not None:
                # カテゴリーデータを取得してスコア計算
                category_data = df[(df['カテゴリー'] == category) & (df['名前'] != '目標値')]
                score = calculate_z_score(latest_val, category_data, metric)
                row_data[f'{metric}_スコア'] = score
            else:
                row_data[f'{metric}_スコア'] = "-"
        
        comparison_data.append(row_data)
    
    return pd.DataFrame(comparison_data)

def create_styled_comparison_table_html(df):
    """スタイリングされたHTMLテーブルを作成（項目ごとの最新・前回値対応）"""
    if df.empty:
    ...
    .sprint-change-positive {
        color: #DC2626;
        font-weight: 600;
    }
    .sprint-change-negative {
        color: #059669;
        font-weight: 600;
    }
    </style>
    ...
    # ヘッダー行の作成
    html += "<thead><tr>"
    html += "<th rowspan='2'>選手名</th>"
    html += "<th rowspan='2'>ID</th>"
    
    # メトリクス名の抽出（修正されたデータフレームから）
    metrics = []
    for col in df.columns:
        if col.endswith('_最新値'):
            metrics.append(col.replace('_最新値', ''))
    
    # メトリクスのヘッダー
    for metric in metrics:
        html += f"<th colspan='4'>{metric}</th>"
    html += "</tr><tr>"
    
    # サブヘッダー
    for _ in metrics:
        html += "<th>最新値</th><th>前回値</th><th>変化</th><th>スコア</th>"
    html += "</tr></thead>"
    ...
    for _, row in df.iterrows():
        ...
        # 選手名とID
        html += f"<td class='player-name'>{row['選手名']}</td>"
        html += f"<td>{row['ID'] if row['ID'] != '' else '-'}</td>"
        
        # 各メトリクスのデータ
        for metric in metrics:
            latest_key = f'{metric}_最新値'
            previous_key = f'{metric}_前回値'
            change_key = f'{metric}_変化'
            score_key = f'{metric}_スコア'
            
            # 最新値
            latest_val = row.get(latest_key, '-')
            html += f"<td>{latest_val}</td>"
            
            # 前回値
            previous_val = row.get(previous_key, '-')
            html += f"<td>{previous_val}</td>"
            
            # 変化（色付き）
            change_val = row.get(change_key, '-')
            change_class = ""
            if change_val != "-" and change_val != "0.00" and change_val != "0.00%":
                if metric in ['20m Sprint(s)', 'Pro Agility', 'CODD']:
                    # Sprint系は値が下がると良い
                    if str(change_val).startswith('+'):
                        change_class = "sprint-change-positive"  # 悪化（赤）
                    elif str(change_val).startswith('-'):
                        change_class = "sprint-change-negative"  # 改善（緑）
                else:
                    # その他は値が上がると良い
                    if str(change_val).startswith('+'):
                        change_class = "change-positive"  # 改善（緑）
                    elif str(change_val).startswith('-'):
                        change_class = "change-negative"  # 悪化（赤）
            ...
            score_class = ""
            try:
                if score_val != "-" and int(score_val) >= 4:
                    score_class = "score-excellent"
            except (ValueError, TypeError):
                pass
            html += f"<td class='{score_class}'>{score_val}</td>"
...
def create_metric_detail_table(df, selected_players, category, metric):
    """特定の項目の詳細テーブルを作成（修正版）"""
    table_data = []
...
def show_team_analysis(df):
    ...
    # 選択された項目ごとにグラフを作成
    create_detailed_analysis_charts(df, selected_players, selected_category, selected_metrics)

def create_detailed_analysis_charts(df, selected_players, category, selected_metrics):
    """選択された各項目の絶対値と変化量のグラフを作成"""
    if not PLOTLY_AVAILABLE:
    ...
    for metric in selected_metrics:
        ...
        # その項目の詳細テーブルを追加
        metric_detail_table = create_metric_detail_table(df, selected_players, category, metric)
...
def create_absolute_values_chart(players_data, metric):
    ...
    target_values = {
        ...
        'CMJ': 55,
        'BJ': 3,
        'BSQ': None,  # 目標値が設定されていない場合
        'BP': None,   # 目標値が設定されていない場合
        'Height': None,
        'Weight': None,
        'Fat%': None,
        '20m Mulch': None,
...
def generate_individual_feedback(player_data, category_data, player_name):
    """個別選手のフィードバックコメントを生成"""
    try:
        # 3つの指標のスコアを計算
...
def generate_team_analysis_pdf(category, selected_players, comparison_table_data, comment_text, df, selected_metrics):
    """比較分析レポートのPDF生成（軽量版 - 全項目表とコメントを削除）"""
    if not PDF_AVAILABLE:
        return None
    ...
    try:
        ...
        doc = SimpleDocTemplate(
            ...
            rightMargin=0.8*cm,
            allowSplitting=1,
            title="SR SHIBUYA Team Analysis Report",
            author="SR SHIBUYA"
        )
        story = []
        
        # 黄色系カラー定義
        yellow_primary = colors.Color(0.9, 0.8, 0.0)
        yellow_secondary = colors.Color(1.0, 0.9, 0.3)
        yellow_light = colors.Color(1.0, 0.95, 0.7)
        ...
        title_style = ParagraphStyle(
            ...
            spaceAfter=10,
            alignment=TA_CENTER, 
            textColor=colors.Color(0.6, 0.5, 0.0)
        )
        ...
        # タイトル
        story.append(Paragraph("SR SHIBUYA", title_style))
        story.append(Paragraph("Team Analysis Report", subtitle_style))
        story.append(Spacer(1, 20))
        
        # カテゴリー情報
        category_info = f"Category: {category} ({len(selected_players)} players)"
        story.append(Paragraph(category_info, normal_style))
        story.append(Spacer(1, 15))
        
        # 選手一覧
        story.append(Paragraph("対象選手一覧", heading_style))
        players_text = "、".join(selected_players)
        story.append(Paragraph(players_text, normal_style))
        story.append(Spacer(1, 20))
        
        # 分析項目一覧
        story.append(Paragraph("分析項目", heading_style))
        metrics_text = "、".join(selected_metrics)
        story.append(Paragraph(metrics_text, normal_style))
        story.append(Spacer(1, 30))
        
        # 各項目の詳細ページ
        for i, metric in enumerate(selected_metrics[:10]):  # 最大10項目
            # 項目タイトル
            story.append(Paragraph(f"項目{i+1}: {metric}", heading_style))
            story.append(Spacer(1, 15))
            
            # 項目別詳細テーブル
            try:
                metric_table_data = create_metric_detail_table(df, selected_players, category, metric)
                
                if not metric_table_data.empty:
                    # テーブルデータの準備
                    pdf_table_data = [['選手名', '最新値', '前回値', '変化']]
                    
                    for _, row in metric_table_data.iterrows():
                        pdf_table_data.append([
                            row['選手名'][:15],  # 名前を15文字で切り詰め
                            str(row['最新値'])[:12],
                            str(row['前回値'])[:12],
                            str(row['変化'])[:10]
                        ])
                    ...
                    # テーブル作成
                    metric_table = Table(pdf_table_data, colWidths=[4*cm, 3.5*cm, 3.5*cm, 3*cm])
                    ...
                    for row_idx in range(1, len(pdf_table_data)):
                        change_val = pdf_table_data[row_idx][3]
                        if change_val != "-" and change_val != "0.00" and change_val != "0.00%":
                            if metric in ['20m Sprint(s)', 'Pro Agility', 'CODD']:
                                if change_val.startswith('-'):
                                    metric_table_style.append(('TEXTCOLOR', (3, row_idx), (3, row_idx), colors.red))
                            else:
                                if change_val.startswith('+'):
                                    metric_table_style.append(('TEXTCOLOR', (3, row_idx), (3, row_idx), colors.red))
                    
                    metric_table.setStyle(TableStyle(metric_table_style))
                    story.append(metric_table)
                    story.append(Spacer(1, 30))
                    
            except Exception as e:
                # 個別テーブルでエラーが発生した場合はスキップ
                story.append(Paragraph(f"{metric}: データ処理エラー", normal_style))
                story.append(Spacer(1, 20))
