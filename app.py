...
def create_multi_trend_chart(historical_data, metrics, title, units):
    """複数メトリクスのトレンドチャートを作成（統一カラー、レスポンシブ対応）"""
    if not PLOTLY_AVAILABLE:
        return None
...
def main():
    ...
    selected_name = st.sidebar.selectbox(
        "選手名を選択してください", 
        ["選手を選択してください"] + list(available_names),
        index=0
    )
    
    # 選手が選択されていない場合はウェルカム画面を表示
    if selected_name == "選手を選択してください":
        show_welcome_screen()
        return
    ...
    with col2:
        ...
        if not all_dates.empty:
            ...
            st.markdown(f'<div class="date-info">測定期間<br>{oldest_date} ~ {latest_date}</div>', unsafe_allow_html=True)
        else:
