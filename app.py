...
def calculate_z_score_rating(current_val, goal_val):
    """目標値からのZスコアで5段階評価を計算"""
    ...
    try:
        ...
        # 目標値を基準にしたZスコア（簡易版：目標値を100%として計算）
        if goal_num != 0:
            achievement_rate = current_num / goal_num
            
            # 5段階評価
            if achievement_rate >= 1.20:  # 120%以上
                return 5, "優秀"
            elif achievement_rate >= 1.10:  # 110%以上
                return 4, "良好"
            elif achievement_rate >= 1.00:  # 100%以上
                return 3, "達成"
            elif achievement_rate >= 0.90:  # 90%以上
                return 2, "もう少し"
            else:  # 90%未満
                return 1, "要努力"
...
def create_radar_chart(player_data, goal_data):
    """レーダーチャートを作成"""
    if not PLOTLY_AVAILABLE:
        return None
    
    # 評価対象メトリクス
    metrics = ['Sprint Momentum', 'LBM/m', 'BW*20m Mulch']
    ...
    for metric in metrics:
        ...
        debug_info.append(f"{metric}: current={current_val}, goal={goal_val}")
        
        rating, text = calculate_z_score_rating(current_val, goal_val)
        ratings.append(rating)
        labels.append(metric)
        rating_texts.append(f"{text} ({rating}/5)")
    ...
    fig.add_trace(go.Scatterpolar(
        r=ratings + [ratings[0]],  # 最初の点に戻る
        theta=labels + [labels[0]],  # 最初のラベルに戻る
        fill='toself',
        name='パフォーマンス評価',
    ...
    return (fig, rating_texts), debug_info
    """特定の項目の測定日を取得"""
    try:
        if column not in data.columns or data.empty:
            return "N/A"
...
def display_metrics_in_columns(df, selected_players, category, metrics):
    ...
    for i in range(0, len(metrics), 2):
        ...
        with col1:
            if i < len(metrics):
                chart = create_team_comparison_single_metric(df, selected_players, category, metrics[i])
                if chart:
                    st.plotly_chart(chart, use_container_width=True, config={'displayModeBar': False})
        
        with col2:
            if i + 1 < len(metrics):
                chart = create_team_comparison_single_metric(df, selected_players, category, metrics[i + 1])
                if chart:
                    st.plotly_chart(chart, use_container_width=True, config={'displayModeBar': False})
