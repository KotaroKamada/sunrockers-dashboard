...
def main():
    ...
    for section_name, section_data in config['sections'].items():
        ...
        if available_metrics:
            ...
            # デバッグ情報（U12専用）
            if selected_category == 'U12':
                st.write("**🔍 U12専用デバッグ情報:**")
                st.write(f"選手データ件数: {len(player_data)}")
                
                # 各項目の生データを確認
                for metric in available_metrics:
                    st.write(f"**{metric}の詳細:**")
                    
                    # 全データから該当項目を確認
                    all_values = player_data[metric].tolist() if metric in player_data.columns else []
                    st.write(f"  全ての値: {all_values}")
                    
                    # dropnaした結果
                    valid_data = player_data.dropna(subset=[metric])
                    st.write(f"  有効データ数: {len(valid_data)}")
                    
                    if len(valid_data) > 0:
                        st.write(f"  有効値: {valid_data[metric].tolist()}")
                        
                        # safe_get_valueの結果
                        result = safe_get_value(player_data, metric)
                        st.write(f"  safe_get_value結果: {result}")
