...
def main():
    ...
    with col1:
        if all(col in player_data.columns for col in ['CMJ', 'RJ']):
            ...
            if len(cmj_data) == 0 and len(rj_data) == 0:
                st.info("CMJとRJのデータがありません")
            else:
                st.info(f"CMJデータ: {len(cmj_data)}件, RJデータ: {len(rj_data)}件")
                ...
                if jump_chart:
                    ...
                    st.error("CMJ & RJグラフの作成に失敗しました")
                    # データの詳細をチェック
                    st.write("CMJデータ詳細:")
                    st.write(cmj_data[['測定日', 'CMJ']].head() if len(cmj_data) > 0 else "データなし")
                    st.write("RJデータ詳細:")
                    st.write(rj_data[['測定日', 'RJ']].head() if len(rj_data) > 0 else "データなし")
        else:
