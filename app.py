...
def show_individual_analysis(df):
    ...
    # Quickness セクション
    with st.expander("### Quickness", expanded=False):
    ...
    # Jump セクション
    with st.expander("### Jump", expanded=False):
        col1, col2 = st.columns(2)
        with col1:
    ...
    # Endurance セクション
    with st.expander("### Endurance", expanded=False):
...
def generate_pdf_report(player_name, player_data, category_data, config):
    ...
    try:
        ...
        if player_id and str(player_id) != '' and str(player_id) != 'nan':
            player_info = f"選手名: {player_name} (ID: {player_id})"
        else:
        ...
        detail_data = [['測定項目', '最新値', '前回値', '変化', 'スコア', '目標値', 'カテゴリー平均']]
        ...
        try:
            ...
            story.append(Paragraph("Key Indicatorsの説明（日本語テキスト）", item_style))
